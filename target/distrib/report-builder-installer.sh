...
if [ -n "${REPORT_BUILDER_INSTALLER_GHE_BASE_URL:-}" ]; then
    ...
    INSTALLER_BASE_URL="${REPORT_BUILDER_INSTALLER_GITHUB_BASE_URL:-https://github.com}"
fi
...
if [ -n "${REPORT_BUILDER_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$REPORT_BUILDER_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${REPORT_BUILDER_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${REPORT_BUILDER_UNMANAGED_INSTALL:-}"
...
USAGE:
    report-builder-installer.sh [OPTIONS]
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set REPORT_BUILDER_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "report-builder-x86_64-unknown-linux-gnu.tar.xz")
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "x86_64-unknown-linux-gnu")
            _archive="report-builder-x86_64-unknown-linux-gnu.tar.xz"
...
install() {
    ...
    if [ -n "${REPORT_BUILDER_INSTALL_DIR:-}" ]; then
        _force_install_dir="$REPORT_BUILDER_INSTALL_DIR"
